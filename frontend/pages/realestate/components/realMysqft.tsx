    ...
    <>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleAdd} style={{ gap: 12, display: "flex", flexDirection: "column" }}>
          ...
          <Form.Item name="projectType" label={<span style={{ fontWeight: 600, color: "#065f46" }}>Project Type</span>} rules={[{ required: true }]} style={{ marginBottom: 16 }}><Input style={{borderRadius: 8, fontSize: 15, padding: "8px 12px", background: "#dcfce7", color: "#065f46", border: "1.5px solid #bbf7d0",}} placeholder="e.g. Plot, Villa, Skyrise"/></Form.Item>
          <Form.Item name="price" label={<span style={{ fontWeight: 600, color: "#065f46" }}>Price</span>} rules={[{ required: true }]} style={{ marginBottom: 16 }}><InputNumber min={100} style={{width: "100%", borderRadius: 8, fontSize: 15, padding: "8px 12px", background: "#dcfce7", color: "#065f46", border: "1.5px solid #bbf7d0",}} placeholder="e.g. 5000"/></Form.Item>
          <Form.Item name="unit" label={<span style={{ fontWeight: 600, color: "#065f46" }}>Unit</span>} rules={[{ required: true, message: 'Please select a unit!' }]} style={{ marginBottom: 16 }}><Select placeholder="Select a unit for the price"><Select.Option value="sqft">Per Square Feet</Select.Option><Select.Option value="sqyd">Per Square Yards</Select.Option></Select></Form.Item>
          
          <div style={{ marginBottom: 18 }}><label style={{fontWeight: 600, color: "#059669", fontSize: 15, marginBottom: 6,}}>Description</label><Input.TextArea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the project..." rows={4} style={{fontSize: 15, fontWeight: 500, color: "#065f46", resize: "vertical", borderRadius: 8, background: "#dcfce7", border: "1.5px solid #bbf7d0", padding: "10px 12px",}} autoSize={{ minRows: 4, maxRows: 8 }}/></div>
          <div style={{marginBottom: 10, transition: "box-shadow 0.2s, border 0.2s", border: dragOver ? "2.5px solid #059669" : "2px dashed #bbf7d0", borderRadius: 14, background: dragOver ? "#bbf7d0" : "#f0fdf4", padding: 18, display: "flex", flexDirection: "column", alignItems: "center", boxShadow: dragOver ? "0 4px 16px #05966933" : "0 2px 8px #bbf7d033",}} onDragOver={e => {e.preventDefault(); setDragOver(true);}} onDragLeave={() => setDragOver(false)} onDrop={e => {e.preventDefault(); setDragOver(false); const files = Array.from(e.dataTransfer.files).filter(f => f.type.startsWith("image/")); setImages(files); setImagePreviews([]); files.forEach((file) => { const reader = new FileReader(); reader.onload = (ev) => {setImagePreviews((prev) => [...prev, ev.target?.result as string,]);}; reader.readAsDataURL(file);});}}><label style={{color: "#059669", fontWeight: 600, fontSize: 15, marginBottom: 8, cursor: "pointer",}}><svg className="w-8 h-8 mb-1" fill="none" stroke="#059669" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg><span>{images.length > 0 ? "Change Images" : "Click or Drag to select images"}</span><input type="file" accept="image/*" multiple onChange={handleImageChange} className="hidden"/></label><div style={{display: "flex", gap: 12, flexWrap: "wrap", marginTop: 8, minHeight: 64,}}>{imagePreviews.map((src, idx) => (<div key={idx} style={{border: "1.5px solid #bbf7d0", borderRadius: 10, overflow: "hidden", width: 64, height: 64, background: "#fff", boxShadow: "0 1px 4px #bbf7d022", display: "flex", alignItems: "center", justifyContent: "center",}}><img src={src} alt={`Project Preview ${idx + 1}`} style={{width: "100%", height: "100%", objectFit: "cover", borderRadius: 10,}}/></div>))}</div>{images.length === 0 && (<span style={{color: "#059669", fontSize: 13, marginTop: 8, fontWeight: 500,}}>No images selected</span>)}</div>
        </Form>
      </Modal>
