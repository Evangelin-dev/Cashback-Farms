...
const DefaultLanding: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* --- Featured Plots Section (Unchanged) --- */}
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-semibold text-gray-800 mb-6 text-center">Featured Plots</h2>
          {isLoading ? <div className="text-center text-gray-500">Loading plots...</div> : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{featuredPlots.map(plot => <DPlotCard key={plot.id} plot={plot} />)}</div>
          )}
          <div className="text-center mt-8"><Link to="/Dplots"><Button variant="outline">View All Plots</Button></Link></div>
        </div>
      </section>

      {/* --- Plot Type Cards Section (Unchanged) --- */}
      <section className="py-8 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 justify-center">
            <div className="bg-white rounded-lg shadow p-0 flex flex-col items-center border border-green-100 hover:shadow-lg transition overflow-hidden relative pb-3">
              <div className="relative w-full h-32"><img src="/images/Residentialplots.jpg" alt="Residential Plots" className="w-full h-full object-cover" /><div className="absolute bottom-0 left-0 w-full h-1/4 bg-gradient-to-t from-white via-white/80 to-transparent" /></div>
              <div className="font-semibold text-green-700 mb-1 mt-2">Residential Plots</div><Button size="sm" variant="outline" onClick={() => window.location.href = '/Dplots?type=residential'}>Explore</Button>
            </div>
            <div className="bg-white rounded-lg shadow p-0 flex flex-col items-center border border-green-100 hover:shadow-lg transition overflow-hidden relative pb-3">
              <div className="relative w-full h-32"><img src="/images/farmplot.jpg" alt="Farms" className="w-full h-full object-cover" /><div className="absolute bottom-0 left-0 w-full h-1/4 bg-gradient-to-t from-white via-white/80 to-transparent" /></div>
              <div className="font-semibold text-green-700 mb-1 mt-2">Farms</div><Button size="sm" variant="outline" onClick={() => window.location.href = '/Dplots?type=farms'}>Explore</Button>
            </div>
            <div className="bg-white rounded-lg shadow p-0 flex flex-col items-center border border-green-100 hover:shadow-lg transition overflow-hidden relative pb-3">
              <div className="relative w-full h-32"><img src="/images/commercialplot.jpeg" alt="Commercial" className="w-full h-full object-cover" /><div className="absolute bottom-0 left-0 w-full h-1/4 bg-gradient-to-t from-white via-white/80 to-transparent" /></div>
              <div className="font-semibold text-green-700 mb-1 mt-2">Commercial</div><Button size="sm" variant="outline" onClick={() => window.location.href = '/Dplots?type=commercial'}>Explore</Button>
            </div>
            <div className="bg-white rounded-lg shadow p-0 flex flex-col items-center border border-green-100 hover:shadow-lg transition overflow-hidden relative pb-3">
              <div className="relative w-full h-32"><img src="/images/rentalplot.jpeg" alt="Rental Yield Plots" className="w-full h-full object-cover" /><div className="absolute bottom-0 left-0 w-full h-1/q from-white via-white/80 to-transparent" /></div>
              <div className="font-semibold text-green-700 mb-1 mt-2">Rental Yield Plots</div><Button size="sm" variant="outline" onClick={() => window.location.href = '/Dplots?type=rental'}>Explore</Button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-100 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? <div className="text-center text-gray-500">Loading professionals...</div> : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{featuredProfessionals.map(prof => <DProfessionalCard key={prof.id} professional={prof} />)}</div>
          )}
