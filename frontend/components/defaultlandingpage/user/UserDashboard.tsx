...
// Mock data for payments tab (as it's not yet connected to an API)
const initialPayments: Payment[] = [
  { id: 'pay_01', date: '2023-10-15', description: 'Down payment for a plot', amount: '₹2,00,000', status: 'Completed' },
  { id: 'pay_02', date: '2023-09-02', description: 'Token advance for a plot', amount: '₹50,000', status: 'Completed' },
];
...
const UserDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!accessToken) {
      setBookingError("You are not logged in.");
      setShortlistError("You are not logged in.");
    ...
    if (activeTab === 'bookings') {
      ...
      apiClient.get<Shortlist[]>('/cart/', { headers })
        ...
        .catch(err => {
          ...
          setShortlistError("Could not load your shortlisted items.");
        })
        .finally(() => {
          setIsLoadingShortlist(false);
        });
  ...
  const handleRemoveShortlist = async (idToRemove: number) => {
    ...
    try {
      await apiClient.delete(`/cart/remove-item/${idToRemove}/`, { headers: { Authorization: `Bearer ${accessToken}` } });
    } catch (err) {
  ...
  const handleClearCart = async () => {
    if (!window.confirm("Are you sure you want to remove all items from your shortlist?")) return;
    const accessToken = localStorage.getItem("access_token");
    try {
      await apiClient.post('/cart/clear/', {}, { headers: { Authorization: `Bearer ${accessToken}` } });
      setShortlists([]);
    } catch (err) {
  ...
  const handleViewDetails = (item: Shortlist) => {
    const pathMap: { [key: string]: string } = {
      'plotlisting': `/book-my-sqft/${item.item_id}`,
      'microplot': `/mysqft-listing/${item.item_id}`,
      'material': `/materials/${item.item_id}`,
    };
    const path = pathMap[item.item_type];
    if (path) {
      navigate(path);
    }
  };
  ...
  const getStatusBadgeClass = (status: BookingStatus | PaymentStatus) => {
  ...
  return (
    <div className="p-4 sm:p-8 bg-gray-50 min-h-screen">
      ...
      <div>
        {/* My Bookings Tab - NOW USES API DATA */}
        {activeTab === 'bookings' && (
          <div>
            {isLoadingBookings ? (
              <div className="text-center p-10"><p className="text-gray-500">Loading your bookings...</p></div>
            ) : bookingError ? (
              <div className="text-center p-10 bg-red-50 text-red-700 rounded-lg"><p>{bookingError}</p></div>
            ) : bookings.length === 0 ? (
              <div className="text-center p-10 bg-gray-100 rounded-lg">
                <p className="font-semibold text-gray-700">You have no bookings yet.</p>
                <p className="text-sm text-gray-500 mt-2">Your confirmed bookings will appear here.</p>
              </div>
            ) : (
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100 flex-grow flex items-end justify-between">
                         <div>
                            ...
                            <p className="text-sm text-gray-500">Area: {booking.booked_area_sqft} sqft</p>
                         </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              {shortlists.length > 0 && !isLoadingShortlist && (
                <button onClick={handleClearCart} className="flex items-center gap-2 text-sm font-semibold text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-3 py-2 rounded-md transition-colors">
            ...
            {isLoadingShortlist ? (
              <div className="text-center p-10"><p className="text-gray-500">Loading your shortlist...</p></div>
            ) : shortlistError ? (
              <div className="text-center p-10 bg-red-50 text-red-700 rounded-lg"><p>{shortlistError}</p></div>
            ) : shortlists.length === 0 ? (
              <div className="text-center p-10 bg-gray-100 rounded-lg">
                ...
                {payments.map(payment => (
                  <tr key={payment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                      <span className={`px-2 inline-flex text-xs leading-5 rounded-full ${getStatusBadgeClass(payment.status)}`}>
                        {payment.status}
                      </span>
                    </td>
                  </tr>
